...
| ④ | `HUMAN #001` 物种卡 `SpeciesCard` | 完成 Aha Moment | 第 3 层：研究档案 |
...
| `GET /species-card` | 生成《HUMAN #001 人类物种卡》 |
...
- 文案语气对齐世界观：写“观察记录”，不写“AI 识别结果”（如「HUMAN #001 摄入透明液体」而不是 `detected: drinking water`）
...
```
观察记录 → 行为归类 → 形成假说 → 更新 HUMAN #001 档案
```

- 展示 `HUMAN #001` 的累计数据（行为归类结果）：
...
- `SpeciesCard` 组件：`HUMAN #001` 人类物种卡，是整场演示的高潮与答辩截图
- 触发方式：点击桌宠旁的「🧬 HUMAN #001」→ 桌宠先**疯狂敲键盘（切到 `THINKING`）**→ 卡片浮现
