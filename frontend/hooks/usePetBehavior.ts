...
const MUTTER = [
  "（思索）……如果把「发呆」也算一种行为，HUMAN #001 今天的成就相当可观",
  ...
  "（自言自语）如果有一天 HUMAN #001 不来了，我就把今天的记录再读一遍",
...
function analysisLines(s: SessionStats, now: number): AnalysisLine[] {
  ...
  return [
    {
      tone: "normal",
      text: `延伸分析：本场观察已进行 ${mins} 分钟，HUMAN #001 主动接触 ${s.taps} 次，互动意愿指数 +${Math.min(90, s.taps * 7)}%`,
    },
    {
      tone: "alert",
      text: `久坐预警：HUMAN #001 已连续静止 ${idleMins} 分钟，痔疮风险较基准上涨 ${(idleMins * 0.05).toFixed(2)}%`,
    },
    {
      tone: "alert",
    ...
    {
      tone: "normal",
      text: `延伸分析：本场记录到 ${s.fasts} 次高速位移，推算 HUMAN #001 今日代谢水平偏高 ${Math.min(40, s.fasts * 6)}%`,
    },
    {
      tone: "normal",
    ...
    {
      tone: "discover",
      text: `趋势推测：按当前接触频率，HUMAN #001 对本观察舱的信任度约 ${Math.min(99, 30 + s.taps * 9)}%，建议继续投喂好奇心`,
    },
  ];
}
