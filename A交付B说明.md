...
8. 点「🧬 观测体 №001」：累计记录 ≥15 条才立案出卡；不足时记录员会怼一句并显示 `样本不足 N/15`。
...
| 主动检测 | 心跳闸门**首次约 70–100s、之后约 120–200s** 触发一条「延伸分析」（只在桌宠空闲且气泡队列空时播）；实测捕获 `延伸分析：本场观察已进行 3 分钟，观测体 №001 主动接触 2 次，互动意愿指数 +14%` |
